$(document).ready(function(){
    ...
    function findRepos() {
        $("repos").each(function() {
            ...
            var updated: string = $(this).data('update');
            if (user) {
    ...
    function getRepos(user: string, updated: string, element: JQuery) {
        $.ajax({
            type: 'GET',
    ...
    function createTable(data, updated: string, user: string, element: JQuery) {
        var rows: number = 0;
        var table = $('<table>');
        var header = $('<tr>');
